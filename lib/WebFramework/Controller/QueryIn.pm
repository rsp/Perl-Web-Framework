package WebFramework::Controller::QueryIn;
...
sub _before {
    ...
    if ($ENV{QUERY_STRING}) {
        $request->{query} ||= {};
        $request->{query}->{params} = WebFramework::Utils::Request::parse_url_query_string($ENV{QUERY_STRING});
        $request->{query}->{string} = $ENV{QUERY_STRING};
    }
    return $request;
}
...
WebFramework::Controller::QueryIn - Perl extension for
